...
from providers.MGnify.metadata import Metadata
from providers.MGnify.scraper import Scraper
from providers.MGnify.gatherer import Gatherer
from providers.MGnify.assemblies import Assemblies as MGnifyAssemblies
from providers.MGnify.sequences import Sequences as MGnifySequences
#from providers.MGnify.taxonomy import SourMashBinning, Kraken2Taxonomy
...
class Maker:
    ...
    def _scrape_metadata(self):
        ...
        scraper = Scraper(dataset=self.dataset, config=self.config)

        print(self.dataset.assembly_present)
        print(self.dataset.targets_file_present)
        # Scrape target files if not already present
        if not (self.dataset.assembly_present and self.dataset.targets_file_present):
            start = time.time()
            scraper.list_target_files()
            elapsed = time.time() - start
            minutes = int(elapsed // 60)
            seconds = elapsed % 60
            print(f"list target files took {minutes} minutes {seconds:.2f} seconds")

        scraper.aggregate_assemblies_metadata()
