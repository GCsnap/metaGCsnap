...
class ParallelTools:
    ...
    def mpiprocess_wrapper(self, parallel_args: list[tuple], func: Callable) -> list:
        ...
        """    
        # Use MPIPoolExecutor only when actually running under mpiexec.
        # mpiexec sets OMPI_COMM_WORLD_SIZE (OpenMPI) or PMI_SIZE (MPICH/Slurm).
        # Checking env vars avoids any MPI call that would trigger singleton-init errors
        # in notebook / interactive sessions where mpiexec is not the parent process.
        under_mpiexec = ('OMPI_COMM_WORLD_SIZE' in os.environ or 'PMI_SIZE' in os.environ)

        # https://mpi4py.readthedocs.io/en/stable/mpi4py.futures.html#parallel-tasks
        executor_cls = MPIPoolExecutor if under_mpiexec else ProcessPoolExecutor
